...
void MarqueeScreen::process() {
    ...
    std::string marqueeText = " " + this->marqueeText + " "; // Add extra spaces for boundary checking
    ...
    while (true) {
        ...
        displayHeader();
        displayFooter();
        drawOnBuffer(x, y, marqueeText, colors[currentColorIndex]); // Use current color
        ...
        if (x <= 0 || x + marqueeText.length() >= screenWidth) {
            dx = -dx;
            currentColorIndex = (currentColorIndex + 1) % (sizeof(colors) / sizeof(colors[0])); // Cycle through colors
        }
